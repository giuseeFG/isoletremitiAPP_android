...
public class DettagliHotelActivity extends FragmentActivity {
	...
	private List<PoiBean> listaTuttiPdi = HomeActivity.getListaAlberghi();
	private String valorePrecedente;

	public void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		requestWindowFeature(Window.FEATURE_NO_TITLE);
		setContentView(R.layout.dettaglihotel);
		...
		Button bottoneRichiediDisponibilita = (Button)findViewById(R.id.buttonActivityDisponibilitaDettagliHotelSecond);
		bottoneRichiediDisponibilita.setText("Richiedi disponibilità");
		...
		LinearLayout layout = (LinearLayout)findViewById(R.id.linearLayoutMappaGoogle);	//serve a rimuovere la mappa se il PDI non ha lat e lng
		if (poiSelezionato.getLat() != 0) {
		...
		inflate.inflateImageHotel(nome, logo);
		...
		if (!email.equals("nd")) {				// se c'è un indirizzo email, posso richiedere la disponibilità
			bottoneRichiediDisponibilita.setOnClickListener(new OnClickListener() {
				public void onClick(View v) {
					startRichiediDisponibilitaActivity();
				}
			});
		}
		else {
			LinearLayout layout2 = (LinearLayout)findViewById(R.id.layoutDettagliHotelSecond);
			layout2.removeView((Button)findViewById(R.id.buttonActivityDisponibilitaDettagliHotelSecond));
		}
		if (!email.equals("nd")) {
	...
	protected void aggiungiPreferito() {
		if (db.checkIfAlreadyExists(id)) {
			System.out.println("ID CHE STO PROVANDO AD INSERIRE: " + id);
			db.close();
			Toast.makeText(this, "Già è stato aggiunto.", Toast.LENGTH_LONG).show();
		}
		else {
			System.out.println("ID CHE HO INSERITO: " + id);
			List<PoiBean> listaHotel = HomeActivity.getListaAlberghi();
			for (PoiBean pb : listaHotel)
	...
	private void startRichiediDisponibilitaActivity() {
		Intent intent = new Intent(this, RichiediDisponibilitaActivity.class);
		intent.putExtra("POI_EMAIL", this.email);
		startActivity(intent);
	}
	@Override
